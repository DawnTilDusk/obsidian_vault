...
- 编译器选择：通过 `CC` 环境变量指定（默认 `gcc`）。示例：
  - 毕昇编译器（示例路径）：`make -C sensor_collectors CC=/opt/bisheng/bin/gcc CFLAGS='-O3 -march=armv8-a -mtune=tsv110 -ffast-math'`
  - GCC/Clang：`make -C sensor_collectors CC=gcc CFLAGS='-O3 -march=armv8-a -mtune=tsv110 -ffast-math'`
...
- 编译器（鲲鹏亲和）：建议毕昇（BiSheng）或 GCC/Clang，示例优化：`-O3 -march=armv8-a -mtune=tsv110 -ffast-math`，可按模块启用 LTO/PGO。
